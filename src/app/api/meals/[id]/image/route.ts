...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!meal?.imageStorageKey) return NextResponse.json({ error: "找不到圖片" }, { status: 404 });

  if (!isStorageKey(meal.imageStorageKey)) {
    const match = meal.imageStorageKey.match(/^data:([^;]+);base64,(.+)$/);
  ...
  const image = await getImageObject(meal.imageStorageKey);
